...
interface PlacePropsTypes {
  title: string;
  photos: string[];
}

const PlaceGallery = ({ title, photos }: PlacePropsTypes) => {
  ...
  if (showAllPhotos) {
    <div className='absolute inset-0 min-h-screen bg-black text-white'>
      <div className='grid gap-4 bg-black p-8'>
        <div>
          <h2 className='mr-48 text-3xl'>Photos of {title}</h2>
          <button
  ...
  return (
    <div className='relative'>
      ...
      <button
        onClick={() => setShowAllPhotos(true)}
        className='absolute bottom-2 right-2 flex gap-1 rounded-2xl bg-white py-2 px-4 shadow-md shadow-gray-500'
      >
        <svg
          xmlns='http://www.w3.org/2000/svg'
          viewBox='0 0 24 24'
